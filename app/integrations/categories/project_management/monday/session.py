...
from app.integrations.categories.project_management.monday.credentials import resolve_api_token
...
def get_token(session: Session, org_id: str, tool_id: str) -> str:
    ...
    if not token:
        raise HTTPException(status_code=400, detail="Missing api_token (Monday personal API token).")
    return token
