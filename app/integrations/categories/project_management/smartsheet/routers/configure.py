...
from app.integrations.categories.project_management.smartsheet.credentials import has_token
...
pm_router = APIRouter(prefix="/project-management/smartsheet", tags=["integrations", "project-management", "smartsheet"])
...
def _cfg(payload: ToolIntegrationPayload, session: Session) -> PmTokenConfigureResponse:
    ...
    return PmTokenConfigureResponse(
        ...
        token_configured=ok,
        next_step="Ready." if ok else "Set api_token in configuration_data.",
...
def flow(org_id: str, tool_id: str, session: Session = Depends(get_db)) -> PmTokenFlowResponse:
    ...
    return PmTokenFlowResponse(
        organization_id=str(row["organization_id"]),
        tool_id=str(row["tool_id"]),
        token_configured=has_token(cfg),
        next_step="Ready." if has_token(cfg) else "Add api_token.",
    )
