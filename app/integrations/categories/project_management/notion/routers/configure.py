...
from app.integrations.categories.project_management.notion.credentials import has_token
...
pm_router = APIRouter(prefix="/project-management/notion", tags=["integrations", "project-management", "notion"])
...
def _cfg(payload: ToolIntegrationPayload, session: Session) -> PmTokenConfigureResponse:
    ...
    return PmTokenConfigureResponse(
        ...
        token_configured=ok,
        next_step="Ready." if ok else "Set integration_secret (internal integration token).",
...
def flow(org_id: str, tool_id: str, session: Session = Depends(get_db)) -> PmTokenFlowResponse:
    ...
    return PmTokenFlowResponse(
        ...
        tool_id=str(row["tool_id"]),
        token_configured=ok,
        next_step="Ready." if ok else "Add integration_secret.",
    )
