...
from app.integrations.categories.project_management.notion.credentials import resolve_token
...
def get_token(session: Session, org_id: str, tool_id: str) -> str:
    ...
    t = resolve_token(cfg)
    if not t:
        raise HTTPException(status_code=400, detail="Missing integration_secret (Notion internal integration token).")
    return t
