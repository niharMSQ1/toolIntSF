"""Evidence collection API for Bitbucket Cloud."""
...
router = APIRouter(prefix="/api/v1/evidence/bitbucket", tags=["evidence", "devtools", "bitbucket"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """Pull evidence from Bitbucket for selected workspaces (OAuth + workspace selection required)."""
    try:
        return run_bitbucket_evidence_collection(
            session,
            org_id=body.org_id,
