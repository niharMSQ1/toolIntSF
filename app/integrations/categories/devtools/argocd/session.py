...
from app.integrations.categories.devtools.argocd.credentials import resolve_base_url, resolve_token
...
def get_api_context(session: Session, org_id: str, tool_id: str) -> dict[str, Any]:
    ...
    if not token:
        raise HTTPException(status_code=400, detail="argocd_token missing in configuration_data.")
    try:
