"""Evidence collection API for Microsoft Defender for Endpoint."""
...
router = APIRouter(
    prefix="/api/v1/evidence/defender-for-endpoint",
    tags=["evidence", "endpoint", "defender-for-endpoint"],
)
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_defender_for_endpoint_evidence_collection(
            session,
            org_id=body.org_id,
