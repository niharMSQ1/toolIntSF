"""Orchestrate Microsoft Defender for Endpoint evidence collection."""
...
from app.integrations.categories.endpoint_security.defender_for_endpoint.collector import (
    collect_for_master,
    defender_for_endpoint_evidence_for_storage,
)
...
from app.integrations.categories.endpoint_security.defender_for_endpoint.seed import (
    ALL_DEFENDER_FOR_ENDPOINT_EVIDENCE_CODES,
...
def run_defender_for_endpoint_evidence_collection_after_configure_background(
    org_id: str, tool_id: str, user_id: str
) -> None:
    ...
    try:
        ...
        logger.exception("Post-configure Defender for Endpoint collection failed org=%s tool=%s", org_id, tool_id)


def run_defender_for_endpoint_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg) or not credentials_valid_shape(cfg):
        raise ValueError(
            "Defender for Endpoint is not ready: set tenant_id, client_id, client_secret, and api_base_url."
        )

    code_filter = evidence_codes if evidence_codes else list(ALL_DEFENDER_FOR_ENDPOINT_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain with source defender_for_endpoint; "
            "run seed_defender_for_endpoint_evidence_masters."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=defender_for_endpoint_evidence_for_storage(content),
