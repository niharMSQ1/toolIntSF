"""Upsert evidence_masters for SentinelOne."""
...
from app.integrations.categories.endpoint_security.sentinelone.seed import SENTINELONE_SEED_ROWS
...
def _api_endpoint_for_row(row: dict[str, str]) -> str:
    return (row.get("api_endpoint") or "").strip() or "sentinelone_rest"


def seed_sentinelone_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    now = datetime.now(timezone.utc)
    touched = 0
    for row in SENTINELONE_SEED_ROWS:
        code = row["code"]
        ...
        if (
            ...
            or (em.source or "") != SENTINELONE_SOURCE
            or (em.api_endpoint or "") != api_endpoint
        ):
            em.name = row["name"]
            em.category = row["category"]
            em.source = SENTINELONE_SOURCE
