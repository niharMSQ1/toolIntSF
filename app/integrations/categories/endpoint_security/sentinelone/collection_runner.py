"""Orchestrate SentinelOne evidence collection."""
...
from app.integrations.categories.endpoint_security.sentinelone.collector import (
    collect_for_master,
    sentinelone_evidence_for_storage,
)
from app.integrations.categories.endpoint_security.sentinelone.constants import SENTINELONE_SOURCE
from app.integrations.categories.endpoint_security.sentinelone.credentials import credentials_valid_shape, ready_for_collection
from app.integrations.categories.endpoint_security.sentinelone.seed import ALL_SENTINELONE_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_sentinelone_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    from app.database import pooled_session

    try:
        with pooled_session() as session:
            run_sentinelone_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
        ...
        logger.exception("Post-configure SentinelOne collection failed org=%s tool=%s", org_id, tool_id)


def run_sentinelone_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg) or not credentials_valid_shape(cfg):
        raise ValueError("SentinelOne is not ready: set api_token and api_base_url (console URL or full /web/api/v2.1 root).")

    code_filter = evidence_codes if evidence_codes else list(ALL_SENTINELONE_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain with source sentinelone; run seed_sentinelone_evidence_masters."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=sentinelone_evidence_for_storage(content),
