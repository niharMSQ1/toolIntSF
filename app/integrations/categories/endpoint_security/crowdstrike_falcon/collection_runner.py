"""Orchestrate CrowdStrike Falcon evidence collection."""
...
from app.integrations.categories.endpoint_security.crowdstrike_falcon.collector import (
    collect_for_master,
    crowdstrike_evidence_for_storage,
)
from app.integrations.categories.endpoint_security.crowdstrike_falcon.constants import CROWDSTRIKE_FALCON_SOURCE
from app.integrations.categories.endpoint_security.crowdstrike_falcon.credentials import credentials_valid_shape, ready_for_collection
from app.integrations.categories.endpoint_security.crowdstrike_falcon.seed import ALL_FALCON_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_crowdstrike_falcon_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    ...
    try:
        ...
        logger.exception("Post-configure CrowdStrike Falcon collection failed org=%s tool=%s", org_id, tool_id)


def run_crowdstrike_falcon_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    cfg = dict(integration.get("configuration_data") or {})
    if not ready_for_collection(cfg) or not credentials_valid_shape(cfg):
        raise ValueError("CrowdStrike Falcon is not ready: set client_id, client_secret, and api_base_url.")

    code_filter = evidence_codes if evidence_codes else list(ALL_FALCON_EVIDENCE_CODES)
    masters = persistence.list_evidence_masters(
        session,
        tool_id=tool_id,
        evidence_codes=code_filter,
        master_name_order=EVIDENCE_MASTER_NAME_ORDER,
        source=CROWDSTRIKE_FALCON_SOURCE,
    )
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain with source crowdstrike_falcon; run seed_crowdstrike_falcon_evidence_masters."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=crowdstrike_evidence_for_storage(content),
