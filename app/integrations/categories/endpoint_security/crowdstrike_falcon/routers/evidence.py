"""Evidence collection API for CrowdStrike Falcon."""
...
router = APIRouter(
    prefix="/api/v1/evidence/crowdstrike-falcon",
    tags=["evidence", "endpoint", "crowdstrike-falcon"],
)
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_crowdstrike_falcon_evidence_collection(
            session,
            org_id=body.org_id,
