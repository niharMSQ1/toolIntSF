"""Upsert evidence_masters for Aqua Security."""
...
from app.integrations.categories.cspm.aqua_security.seed import AQUA_CSPM_SEED_ROWS
...
def _api_endpoint_for_row(row: dict[str, str]) -> str:
    return (row.get("api_endpoint") or "").strip() or "aqua_rest"


def seed_aqua_security_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    now = datetime.now(timezone.utc)
    touched = 0
    for row in AQUA_CSPM_SEED_ROWS:
        code = row["code"]
        ...
        if (
            ...
            or (em.source or "") != AQUA_SECURITY_SOURCE
            or (em.api_endpoint or "") != api_endpoint
        ):
            em.name = row["name"]
            em.category = row["category"]
            em.source = AQUA_SECURITY_SOURCE
