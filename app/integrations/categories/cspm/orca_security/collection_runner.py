"""Orchestrate Orca Security evidence collection."""
...
from app.integrations.categories.cspm.orca_security.collector import collect_for_master, orca_evidence_for_storage
from app.integrations.categories.cspm.orca_security.constants import ORCA_SECURITY_SOURCE
from app.integrations.categories.cspm.orca_security.credentials import credentials_valid_shape, ready_for_collection
from app.integrations.categories.cspm.orca_security.seed import ALL_ORCA_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_orca_security_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    from app.database import pooled_session

    try:
        with pooled_session() as session:
            run_orca_security_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
        ...
        logger.exception("Post-configure Orca Security collection failed org=%s tool=%s", org_id, tool_id)


def run_orca_security_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg) or not credentials_valid_shape(cfg):
        raise ValueError("Orca Security is not ready: set api_token and api host / base URL.")

    code_filter = evidence_codes if evidence_codes else list(ALL_ORCA_EVIDENCE_CODES)
    masters = persistence.list_evidence_masters(
        session,
        tool_id=tool_id,
        evidence_codes=code_filter,
        master_name_order=EVIDENCE_MASTER_NAME_ORDER,
        source=ORCA_SECURITY_SOURCE,
    )
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain with source orca_security; run seed_orca_security_evidence_masters."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=orca_evidence_for_storage(content),
