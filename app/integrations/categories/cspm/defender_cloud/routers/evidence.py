"""Evidence collection for Microsoft Defender for Cloud."""
...
router = APIRouter(prefix="/api/v1/evidence/defender-cloud", tags=["evidence", "cspm", "defender-cloud"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """Pull evidence via Azure ARM Microsoft.Security APIs."""
    try:
        return run_defender_cloud_evidence_collection(
            session,
            org_id=body.org_id,
