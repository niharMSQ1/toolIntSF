"""Upsert evidence_masters for Prisma Cloud (CSPM)."""
...
from app.integrations.categories.cspm.prisma_cloud.seed import PRISMA_CSPM_SEED_ROWS
...
def _api_endpoint_for_row(row: dict[str, str]) -> str:
    return (row.get("api_endpoint") or "").strip() or "prisma_cloud_rest"


def seed_prisma_cloud_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    now = datetime.now(timezone.utc)
    touched = 0
    for row in PRISMA_CSPM_SEED_ROWS:
        code = row["code"]
        ...
        if (
            ...
            or (em.source or "") != PRISMA_CLOUD_SOURCE
            or (em.api_endpoint or "") != api_endpoint
        ):
            em.name = row["name"]
            em.category = row["category"]
            em.source = PRISMA_CLOUD_SOURCE
