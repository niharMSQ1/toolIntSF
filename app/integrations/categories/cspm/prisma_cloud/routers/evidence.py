"""Evidence collection API for Prisma Cloud CSPM."""
...
router = APIRouter(prefix="/api/v1/evidence/prisma-cloud", tags=["evidence", "cspm", "prisma-cloud"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """Pull evidence via Prisma Cloud REST APIs (JWT in x-redlock-auth)."""
    try:
        return run_prisma_cloud_evidence_collection(
            session,
            org_id=body.org_id,
