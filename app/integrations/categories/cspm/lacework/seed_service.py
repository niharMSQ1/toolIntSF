"""Upsert evidence_masters for Lacework."""
...
from app.integrations.categories.cspm.lacework.seed import LACEWORK_CSPM_SEED_ROWS
...
def _api_endpoint_for_row(row: dict[str, str]) -> str:
    return (row.get("api_endpoint") or "").strip() or "lacework_rest"


def seed_lacework_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    now = datetime.now(timezone.utc)
    touched = 0
    for row in LACEWORK_CSPM_SEED_ROWS:
        code = row["code"]
