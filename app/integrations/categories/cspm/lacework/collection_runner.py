"""Orchestrate Lacework evidence collection."""
...
from app.integrations.categories.cspm.lacework.collector import collect_for_master, lacework_evidence_for_storage
from app.integrations.categories.cspm.lacework.constants import LACEWORK_SOURCE
from app.integrations.categories.cspm.lacework.credentials import credentials_valid_shape, ready_for_collection
from app.integrations.categories.cspm.lacework.seed import ALL_LACEWORK_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_lacework_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    from app.database import pooled_session

    try:
        with pooled_session() as session:
            run_lacework_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
        ...
        logger.exception("Post-configure Lacework collection failed org=%s tool=%s", org_id, tool_id)


def run_lacework_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg) or not credentials_valid_shape(cfg):
        raise ValueError("Lacework is not ready: set account (subdomain), key_id, and secret.")

    code_filter = evidence_codes if evidence_codes else list(ALL_LACEWORK_EVIDENCE_CODES)
    masters = persistence.list_evidence_masters(
        session,
        tool_id=tool_id,
        evidence_codes=code_filter,
        master_name_order=EVIDENCE_MASTER_NAME_ORDER,
        source=LACEWORK_SOURCE,
    )
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain with source lacework; run seed_lacework_evidence_masters."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=lacework_evidence_for_storage(content),
