"""Lacework: account + API key id + secret."""
...
from app.integrations.categories.cspm.lacework.api_client import LaceworkApiError, validate_credentials
from app.integrations.categories.cspm.lacework.collection_runner import (
    run_lacework_evidence_collection_after_configure_background,
)
...
from app.schemas import (
    LaceworkConfigureResponse,
    LaceworkFlowResponse,
    ToolIntegrationPayload,
    ToolIntegrationResponse,
)

router = APIRouter(prefix="/api/v1/integrations/cspm/lacework", tags=["integrations", "cspm", "lacework"])
...
def _normalize_configuration_data(raw: dict[str, Any]) -> dict[str, Any]:
    data = dict(raw)
    if str(data.get("provider_key", "")).strip().lower() == "lacework":
        data["provider_key"] = "lacework"
    return data


@router.post("/configure", response_model=LaceworkConfigureResponse)
def configure_lacework(
    ...
    if credentials_valid_shape(cfg):
        try:
            validate_credentials(
                ...
                resolve_key_id(cfg) or "",
                resolve_secret(cfg) or "",
            )
            cred_ok = True
        except LaceworkApiError as e:
            raise HTTPException(status_code=400, detail=str(e)) from e
    ...
    return LaceworkConfigureResponse(
        id=str(row["id"]),
        organization_id=oid,
        tool_id=tid,
        ...
        next_step=(
            "Integration ready. Lacework evidence collection started in the background."
            if rdy
            else "Set account (subdomain), key_id, and secret, then POST /configure again."
        ),
        configuration_data=masked,
    )


@router.get("/flow", response_model=LaceworkFlowResponse)
def lacework_flow(org_id: str, tool_id: str, session: Session = Depends(get_db)) -> LaceworkFlowResponse:
