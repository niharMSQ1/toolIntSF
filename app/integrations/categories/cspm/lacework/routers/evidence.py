"""Evidence collection API for Lacework."""
...
router = APIRouter(prefix="/api/v1/evidence/lacework", tags=["evidence", "cspm", "lacework"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_lacework_evidence_collection(
            session,
            org_id=body.org_id,
