"""Evidence collection API for Sysdig Secure."""
...
router = APIRouter(prefix="/api/v1/evidence/sysdig-secure", tags=["evidence", "cspm", "sysdig-secure"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_sysdig_secure_evidence_collection(
            session,
            org_id=body.org_id,
