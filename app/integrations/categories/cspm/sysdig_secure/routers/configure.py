"""Sysdig Secure: regional API URL + Bearer API token."""
...
from app.integrations.categories.cspm.sysdig_secure.api_client import SysdigSecureApiError, validate_api_token
from app.integrations.categories.cspm.sysdig_secure.collection_runner import (
    run_sysdig_secure_evidence_collection_after_configure_background,
)
...
from app.schemas import (
    SysdigSecureConfigureResponse,
    SysdigSecureFlowResponse,
    ToolIntegrationPayload,
    ToolIntegrationResponse,
)

router = APIRouter(prefix="/api/v1/integrations/cspm/sysdig-secure", tags=["integrations", "cspm", "sysdig-secure"])
...
def _normalize_configuration_data(raw: dict[str, Any]) -> dict[str, Any]:
    data = dict(raw)
    if str(data.get("provider_key", "")).strip().lower() == "sysdig_secure":
        data["provider_key"] = "sysdig_secure"
    return data


@router.post("/configure", response_model=SysdigSecureConfigureResponse)
def configure_sysdig_secure(
    ...
    if credentials_valid_shape(cfg):
        try:
            ...
            cred_ok = True
        except SysdigSecureApiError as e:
            raise HTTPException(status_code=400, detail=str(e)) from e
    ...
    return SysdigSecureConfigureResponse(
        id=str(row["id"]),
        organization_id=oid,
        tool_id=tid,
        ...
        next_step=(
            "Integration ready. Sysdig Secure evidence collection started in the background."
            if rdy
            else "Set api_token and api_base_url (region), then POST /configure again."
        ),
        configuration_data=masked,
    )


@router.get("/flow", response_model=SysdigSecureFlowResponse)
def sysdig_flow(org_id: str, tool_id: str, session: Session = Depends(get_db)) -> SysdigSecureFlowResponse:
    ...
    return SysdigSecureFlowResponse(
        organization_id=str(row["organization_id"]),
        tool_id=str(row["tool_id"]),
        credentials_valid=cred_ok,
