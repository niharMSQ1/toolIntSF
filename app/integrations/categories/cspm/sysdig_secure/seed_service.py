"""Upsert evidence_masters for Sysdig Secure."""
...
from app.integrations.categories.cspm.sysdig_secure.seed import SYSDIG_CSPM_SEED_ROWS
...
def _api_endpoint_for_row(row: dict[str, str]) -> str:
    return (row.get("api_endpoint") or "").strip() or "sysdig_rest"


def seed_sysdig_secure_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    now = datetime.now(timezone.utc)
    touched = 0
    for row in SYSDIG_CSPM_SEED_ROWS:
        code = row["code"]
        ...
        if (
            ...
            or (em.source or "") != SYSDIG_SECURE_SOURCE
            or (em.api_endpoint or "") != api_endpoint
        ):
            em.name = row["name"]
            em.category = row["category"]
            em.source = SYSDIG_SECURE_SOURCE
