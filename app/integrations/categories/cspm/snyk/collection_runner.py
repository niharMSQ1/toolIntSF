"""Orchestrate Snyk evidence collection."""
...
from app.integrations.categories.cspm.snyk.collector import collect_for_master, snyk_evidence_for_storage
from app.integrations.categories.cspm.snyk.credentials import has_credentials_for_api, ready_for_collection
from app.integrations.categories.cspm.snyk.seed import ALL_SNYK_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
from app.integrations.categories.cspm.snyk.token_refresh import ensure_fresh_snyk_credentials
...
def run_snyk_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    """Run full Snyk collection in a new DB session (FastAPI BackgroundTasks)."""
    from app.database import pooled_session

    try:
        with pooled_session() as session:
            out = run_snyk_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure Snyk evidence collection finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure Snyk evidence collection failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_snyk_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    cfg = ensure_fresh_snyk_credentials(session, integration)
    if not isinstance(cfg, dict):
        raise ValueError("Invalid configuration_data")
    if not ready_for_collection(cfg):
        raise ValueError(
            "Snyk is not ready: set auth (api_key, access_token, or OAuth client credentials), "
            "and org_ids or group_id, then POST /configure."
        )
    if not has_credentials_for_api(cfg):
        raise ValueError("Missing Snyk credentials.")

    code_filter = evidence_codes if evidence_codes else list(ALL_SNYK_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain; seed evidence_masters (seed_snyk_evidence_masters) before collect."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=snyk_evidence_for_storage(content),
