"""Evidence collection API for Snyk."""
...
router = APIRouter(prefix="/api/v1/evidence/snyk", tags=["evidence", "cspm", "snyk"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """Pull evidence via Snyk REST API (token + org or group scope)."""
    try:
        return run_snyk_evidence_collection(
            session,
            org_id=body.org_id,
