"""Evidence collection API for Wiz CSPM."""
...
router = APIRouter(prefix="/api/v1/evidence/wiz", tags=["evidence", "cspm", "wiz"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """Pull evidence via Wiz GraphQL (service account)."""
    try:
        return run_wiz_evidence_collection(
            session,
            org_id=body.org_id,
