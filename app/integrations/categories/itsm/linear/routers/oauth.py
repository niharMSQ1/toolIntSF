"""Linear OAuth authorize + callback routes."""
...
from app.integrations.categories.itsm.linear.credentials import resolve_oauth_credentials, resolve_redirect_uri
from app.integrations.categories.itsm.linear.oauth import (
...
from app.schemas import AuthorizeResponse, LinearOAuthCallbackResponse

router = APIRouter(tags=["oauth", "itsm", "linear"])
...
@router.get("/api/v1/oauth/linear/authorize", response_model=AuthorizeResponse)
@router.get("/api/v1/integrations/linear/connect", response_model=AuthorizeResponse)
def linear_authorize(
    org_id: str,
    tool_id: str,
...
def _linear_oauth_callback_impl(
    code: str,
    state: str,
    session: Session,
    ...
    *,
    error: str | None,
) -> RedirectResponse | LinearOAuthCallbackResponse:
    ...
    if collection_started:
        next_msg = (
            "OAuth complete. Evidence collection from Linear is running automatically in the background. "
...
@router.get("/itsm/linear/callback", response_model=None)
@router.get("/integrations/linear/callback", response_model=None)
def linear_oauth_callback(
    ...
    return _linear_oauth_callback_impl(code, state, session, background_tasks, error=error)
