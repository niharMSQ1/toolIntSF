"""Evidence collection API for Linear-backed integrations."""
...
router = APIRouter(prefix="/api/v1/evidence/linear", tags=["evidence", "itsm", "linear"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """
    Pull evidence from Linear using stored OAuth tokens (refreshes access_token if needed).
    After OAuth, collection also runs automatically in the background — use this to re-run or debug.
    """
    try:
        return run_linear_evidence_collection(
            session,
            org_id=body.org_id,
