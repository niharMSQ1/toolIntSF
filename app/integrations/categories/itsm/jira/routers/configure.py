"""Jira Cloud: configure, flow, status, token refresh (ITSM category)."""
...
from app.schemas import (
    ...
    JiraFlowResponse,
    JiraRefreshTokensBody,
    JiraRefreshTokensResponse,
    ToolIntegrationPayload,
    ToolIntegrationResponse,
)

router = APIRouter(prefix="/api/v1/integrations/jira", tags=["integrations", "itsm", "jira"])
itsm_router = APIRouter(prefix="/itsm/jira", tags=["integrations", "itsm", "jira"])
...
def _configure_jira_response(
    ...
    return JiraConfigureResponse(
        ...
        next_step=(
            ...
            "automatically in the background (no separate POST to /api/v1/evidence/jira/collect required)."
        ),
        configuration_data=masked,
    )


@router.post("/refresh-tokens", response_model=JiraRefreshTokensResponse)
def jira_refresh_tokens(payload: JiraRefreshTokensBody, session: Session = Depends(get_db)) -> JiraRefreshTokensResponse:
    row = persistence.get_integration(session, payload.org_id, payload.tool_id)
    if not row:
        raise HTTPException(status_code=404, detail="Integration not found.")
    try:
        cfg, did_refresh = refresh_jira_access_tokens(session, row, force=payload.force)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
    oid = str(row["organization_id"])
    tid = str(row["tool_id"])
    masked = _mask_configuration_data(cfg)
    msg = (
        "Access token refreshed from Atlassian."
        if did_refresh
        else "Access token still valid; Atlassian token API was not called. Use force=true to refresh anyway."
    )
    return JiraRefreshTokensResponse(
        ok=True,
        organization_id=oid,
        tool_id=tid,
...
def configure_jira(
    ...
    return _configure_jira_response(payload, session, background_tasks)
...
@router.get("/flow", response_model=JiraFlowResponse)
def jira_flow_status(org_id: str, tool_id: str, session: Session = Depends(get_db)) -> JiraFlowResponse:
    ...
    return JiraFlowResponse(
        ...
        redirect_uri=redirect,
        next_step=(
            "Open authorization_url in a browser, approve Atlassian access. "
            "You must be redirected to redirect_uri on THIS FastAPI host (same port as uvicorn). "
            "If your redirect_uri does not match the Atlassian app callback URL, update the developer console "
