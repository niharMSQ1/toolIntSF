"""Orchestrate AWS evidence collection (AssumeRole per run)."""
...
from app.integrations.categories.cloud.aws.collector import aws_evidence_for_storage, collect_for_master
from app.integrations.categories.cloud.aws.credentials import ready_for_collection
from app.integrations.categories.cloud.aws.seed import ALL_AWS_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
from app.integrations.categories.cloud.aws.session import AwsAssumeRoleError, assume_role_session
...
def run_aws_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    ...
    try:
        with pooled_session() as session:
            out = run_aws_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure AWS evidence collection finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure AWS evidence collection failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_aws_evidence_collection(
    ...
    *,
    ...
    if not ready_for_collection(cfg):
        raise ValueError(
            "AWS is not ready: set a valid IAM role ARN in configuration_data.role_arn (and optional external_id, region)."
        )

    try:
        boto_session = assume_role_session(cfg)
    except AwsAssumeRoleError as e:
        raise ValueError(str(e)) from e

    code_filter = evidence_codes if evidence_codes else list(ALL_AWS_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain; run seed_aws_evidence_masters(session, tool_id) before collect."
        )
    ...
    for master in masters:
        ...
        try:
            content = collect_for_master(master, cfg, boto_session=boto_session)
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=aws_evidence_for_storage(content),
