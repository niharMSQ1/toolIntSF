"""Orchestrate GCP evidence collection."""
...
from app.integrations.categories.cloud.gcp.collector import collect_for_master, gcp_evidence_for_storage
from app.integrations.categories.cloud.gcp.credentials import ready_for_collection
from app.integrations.categories.cloud.gcp.evidence_map import ALL_GCP_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_gcp_evidence_collection_after_configure_background(org_id: str, tool_id: str, user_id: str) -> None:
    from app.database import pooled_session

    try:
        with pooled_session() as session:
            out = run_gcp_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure GCP evidence collection finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure GCP evidence collection failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_gcp_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg):
        raise ValueError("GCP is not ready: set project_id and service_account_json in configuration_data.")
    try:
        validate_gcp_access(cfg)
    except GcpAuthError as e:
        raise ValueError(str(e)) from e

    code_filter = evidence_codes if evidence_codes else list(ALL_GCP_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain; seed cloud evidence masters for this domain first."
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=gcp_evidence_for_storage(content),
