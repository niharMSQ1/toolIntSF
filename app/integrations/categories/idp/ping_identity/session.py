...
from app.integrations.categories.idp.ping_identity.credentials import resolve_access_token
...
def get_config_and_token(session: Session, org_id: str, tool_id: str) -> tuple[dict[str, Any], str]:
    ...
    if not token:
        raise HTTPException(status_code=400, detail="access_token missing; complete configure with Worker credentials.")
    return cfg, token
