"""Seed evidence_masters for Ping Identity (IAM) when codes are not already present globally."""
...
from app.integrations.categories.idp.ping_identity.seed import PING_IAM_SEED_ROWS
...
def seed_ping_identity_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    count = 0
    for row in PING_IAM_SEED_ROWS:
