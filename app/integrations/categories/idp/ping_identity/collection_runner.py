"""Orchestrate Ping Identity (PingOne) IAM evidence collection."""
...
from app.integrations.categories.idp.ping_identity.collector import collect_for_master, ping_evidence_for_storage
...
from app.integrations.categories.idp.ping_identity.seed import ALL_PING_IAM_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_ping_identity_evidence_collection_after_configure_background(
    org_id: str,
    tool_id: str,
    user_id: str,
) -> None:
    ...
    try:
        with pooled_session() as session:
            out = run_ping_identity_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure Ping Identity evidence collection finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure Ping Identity evidence collection failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_ping_identity_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg):
        raise ValueError(
            "Set pingone_environment_id and PingOne Worker client_id/client_secret (or access_token), then POST /configure.",
        )
    ...
    code_filter = evidence_codes if evidence_codes else list(ALL_PING_IAM_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError(
            "No evidence_masters for this tool's domain; seed IAM evidence (e.g. ping_identity seed) before collect.",
        )
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=ping_evidence_for_storage(content),
...
def validate_ping_credentials(cfg: dict[str, Any]) -> bool:
    try:
        from app.integrations.categories.idp.ping_identity import api_client
        from app.integrations.categories.idp.ping_identity.credentials import resolve_access_token as _tok
