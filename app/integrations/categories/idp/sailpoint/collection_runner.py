"""Orchestrate SailPoint IdentityNow IAM evidence collection."""
...
from app.integrations.categories.idp.sailpoint.collector import collect_for_master, sailpoint_evidence_for_storage
from app.integrations.categories.idp.sailpoint.credentials import ready_for_collection, resolve_access_token, resolve_api_base
from app.integrations.categories.idp.sailpoint.seed import ALL_SAILPOINT_IAM_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_sailpoint_evidence_collection_after_configure_background(
    org_id: str,
    tool_id: str,
    user_id: str,
) -> None:
    ...
    try:
        with pooled_session() as session:
            out = run_sailpoint_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure SailPoint evidence finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure SailPoint evidence failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_sailpoint_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg):
        raise ValueError(
            "Set sailpoint_base_url and client_id/client_secret (or access_token), then POST /configure.",
        )

    resolve_api_base(cfg)
    resolve_access_token(cfg)

    code_filter = evidence_codes if evidence_codes else list(ALL_SAILPOINT_IAM_EVIDENCE_CODES)
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=sailpoint_evidence_for_storage(content),
...
def validate_sailpoint_credentials(cfg: dict[str, Any]) -> bool:
    try:
