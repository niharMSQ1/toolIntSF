"""Evidence collection for ForgeRock."""
...
router = APIRouter(prefix="/api/v1/evidence/forgerock", tags=["evidence", "idp", "forgerock"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_forgerock_evidence_collection(
            session,
            org_id=body.org_id,
