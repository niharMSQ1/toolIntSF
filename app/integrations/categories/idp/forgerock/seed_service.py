"""Seed evidence_masters for ForgeRock (IAM) when codes are not already present."""
...
from app.integrations.categories.idp.forgerock.seed import FORGEROCK_IAM_SEED_ROWS
...
def seed_forgerock_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    count = 0
    for row in FORGEROCK_IAM_SEED_ROWS:
