"""Evidence collection for JumpCloud."""
...
router = APIRouter(prefix="/api/v1/evidence/jumpcloud", tags=["evidence", "idp", "jumpcloud"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_jumpcloud_evidence_collection(
            session,
            org_id=body.org_id,
