"""Orchestrate JumpCloud IAM evidence collection."""
...
from app.integrations.categories.idp.jumpcloud.collector import collect_for_master, jumpcloud_evidence_for_storage
from app.integrations.categories.idp.jumpcloud.credentials import ready_for_collection, resolve_api_key
from app.integrations.categories.idp.jumpcloud.seed import ALL_JUMPCLOUD_IAM_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_jumpcloud_evidence_collection_after_configure_background(
    org_id: str,
    tool_id: str,
    user_id: str,
) -> None:
    ...
    try:
        with pooled_session() as session:
            out = run_jumpcloud_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure JumpCloud evidence finished org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception(
            "Post-configure JumpCloud evidence failed org=%s tool=%s user_id=%s",
            org_id,
            tool_id,
            user_id,
        )
...
def run_jumpcloud_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg):
        raise ValueError(
            "Set jumpcloud_api_key, then POST /configure.",
        )

    resolve_api_key(cfg)

    code_filter = evidence_codes if evidence_codes else list(ALL_JUMPCLOUD_IAM_EVIDENCE_CODES)
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=jumpcloud_evidence_for_storage(content),
...
def validate_jumpcloud_credentials(cfg: dict[str, Any]) -> bool:
    try:
        from app.integrations.categories.idp.jumpcloud import api_client

        k = resolve_api_key(cfg)
        return api_client.validate_connection(cfg, k)
    except Exception:
        return False
