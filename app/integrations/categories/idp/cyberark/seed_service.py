"""Seed evidence_masters for CyberArk Identity (IAM) when codes are not already present."""
...
from app.integrations.categories.idp.cyberark.seed import CYBER_IAM_SEED_ROWS
...
def seed_cyberark_evidence_masters(session: Session, tool_id: str) -> int:
    did = get_domain_id_for_tool(session, tool_id)
    count = 0
    for row in CYBER_IAM_SEED_ROWS:
