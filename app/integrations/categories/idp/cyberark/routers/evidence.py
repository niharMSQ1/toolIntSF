"""Evidence collection for CyberArk Identity."""
...
router = APIRouter(prefix="/api/v1/evidence/cyberark-identity", tags=["evidence", "idp", "cyberark"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    try:
        return run_cyberark_evidence_collection(
            session,
            org_id=body.org_id,
