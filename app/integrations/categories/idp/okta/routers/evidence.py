"""Evidence collection API for Okta IAM integrations."""
...
router = APIRouter(prefix="/api/v1/evidence/okta", tags=["evidence", "idp", "okta"])
...
def collect_evidence(body: CollectEvidenceBody, session: Session = Depends(get_db)) -> CollectEvidenceResponse:
    """
    Pull evidence from Okta Admin API using stored org_domain and api_token.
    After a successful /configure, collection also runs in the background; use this to re-run or debug.
    """
    try:
        return run_okta_evidence_collection(
            session,
            org_id=body.org_id,
