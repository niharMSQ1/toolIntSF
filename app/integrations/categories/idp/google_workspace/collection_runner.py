"""Orchestrate Google Workspace IAM evidence collection."""
...
from app.integrations.categories.idp.google_workspace.collector import collect_for_master, google_workspace_evidence_for_storage
from app.integrations.categories.idp.google_workspace.credentials import (
    ready_for_collection,
    resolve_access_token,
    resolve_workspace_domain,
)
from app.integrations.categories.idp.google_workspace.seed import ALL_GOOGLE_WORKSPACE_IAM_EVIDENCE_CODES, EVIDENCE_MASTER_NAME_ORDER
...
def run_google_workspace_evidence_collection_after_configure_background(
    org_id: str,
    tool_id: str,
    user_id: str,
) -> None:
    ...
    try:
        with pooled_session() as session:
            out = run_google_workspace_evidence_collection(
                session,
                org_id=org_id,
                tool_id=tool_id,
                user_id=user_id,
            ...
            logger.info(
                "Post-configure Google Workspace evidence org=%s tool=%s success=%s/%s",
                org_id,
                tool_id,
                ok,
                len(out.results),
        ...
        logger.exception("Post-configure Google Workspace evidence failed org=%s tool=%s", org_id, tool_id)


def run_google_workspace_evidence_collection(
    session: Session,
    *,
    org_id: str,
    tool_id: str,
    ...
    if not ready_for_collection(cfg):
        raise ValueError(
            "Set google_workspace_domain and OAuth refresh_token + client (or access_token), then POST /configure.",
        )

    resolve_workspace_domain(cfg)
    resolve_access_token(cfg)

    code_filter = evidence_codes if evidence_codes else list(ALL_GOOGLE_WORKSPACE_IAM_EVIDENCE_CODES)
    ...
    if not masters:
        raise ValueError("No evidence_masters for this tool's domain; seed IAM evidence before collect.")
    ...
    for master in masters:
        ...
        try:
            ...
            persistence.insert_evidence_collection(
                ...
                user_id=user_id,
                tool_id=tool_id,
                tool_evidence=google_workspace_evidence_for_storage(content),
...
def validate_google_workspace_credentials(cfg: dict[str, Any]) -> bool:
    try:
