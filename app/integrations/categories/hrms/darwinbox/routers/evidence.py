"""Explicit Darwinbox evidence collection endpoint."""
...
router = APIRouter(prefix="/api/v1/evidence", tags=["evidence", "hrms", "darwinbox"])


@router.post("/darwinbox/collect", response_model=CollectEvidenceResponse)
def collect_darwinbox_evidence(
    body: CollectEvidenceBody,
    session: Session = Depends(get_db),
) -> CollectEvidenceResponse:
    try:
        return run_darwinbox_evidence_collection(
            session,
            org_id=body.org_id,
