"""UKG personnel payloads to unified HR schema."""
...
def ukg_extract_people(body: Any) -> list[dict[str, Any]]:
...
def ukg_person_to_employee(raw: dict[str, Any]) -> HREmployee:
    pid = str(raw.get("id") or raw.get("employeeId") or raw.get("personId") or "")
    name = raw.get("fullName") or raw.get("name") or raw.get("preferredName")
    return HREmployee(
        id=pid,
...
def ukg_webhook_to_event(payload: dict[str, Any]) -> HREvent:
    return HREvent(
        id=None,
