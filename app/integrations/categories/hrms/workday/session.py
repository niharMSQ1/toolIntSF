"""Load Workday integration and access token."""
...
from app.integrations.categories.hrms.workday.credentials import resolve_access_token
...
def get_config_and_token(session: Session, org_id: str, tool_id: str) -> tuple[dict[str, Any], str]:
    ...
    if not token:
        raise HTTPException(
            status_code=400,
            detail="access_token missing; complete OAuth client credentials in configuration or POST /configure.",
        )
    return cfg, token
