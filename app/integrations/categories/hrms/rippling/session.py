...
from app.integrations.categories.hrms.rippling.credentials import resolve_bearer_token
...
def get_config_and_token(session: Session, org_id: str, tool_id: str) -> tuple[dict[str, Any], str]:
    ...
    if not token:
        raise HTTPException(status_code=400, detail="access_token or rippling_api_key missing.")
    return cfg, token
