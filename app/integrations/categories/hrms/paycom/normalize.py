"""Paycom API payloads to unified HR schema."""
...
def paycom_extract_employees(body: Any) -> list[dict[str, Any]]:
...
def paycom_row_to_employee(raw: dict[str, Any]) -> HREmployee:
    pid = str(raw.get("id") or raw.get("employeeId") or raw.get("eecode") or "")
...
def paycom_webhook_to_event(payload: dict[str, Any]) -> HREvent:
    return HREvent(
        id=None,
