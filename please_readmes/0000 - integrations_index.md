...
| **Microsoft Entra** (commercial) | IDP | `microsoft_entra` | `microsoft_entra` | [0004 - microsoft_entra_integration.md](0004%20-%20microsoft_entra_integration.md) |
| **Microsoft Entra** (GCC High) | IDP | `microsoft_entra_gcc_high` | `microsoft_entra_gcc_high` | [0004 - microsoft_entra_integration.md](0004%20-%20microsoft_entra_integration.md) |
...
| **Okta** | IAM / IDP | `okta` | `okta` | [0008 - okta_iam_integration.md](0008%20-%20okta_iam_integration.md) |
...
- If **`provider_key` is omitted**, it is inferred from **`evidence_masters.source`** for that tool’s **`domain_id`**, as long as the domain has a **single** unambiguous source (see [`sync_dispatch.py`](../app/integrations/core/sync_dispatch.py)).
- If multiple sources exist on one domain, pass the matching **`provider_key`** explicitly.
...
3. **G3** — Configure seeds **`evidence_masters`** for the tool’s **`domain_id`** (from **`tools.domain_id`**) when codes are not already present globally (per product rules).
4. **OAuth tools** — User completes browser OAuth where applicable; callback stores tokens in **`configuration_data`**.
5. **G4** — Evidence collection (background after configure and/or **`POST …/collect`** / **`POST /integrations/sync`**) calls vendor APIs, **`upsert_evidence_full_replace`** on **`evidence`**, **`insert_evidence_collection`** for each run.
